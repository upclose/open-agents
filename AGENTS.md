...
- Ink's `<Static>` component accumulates content in an internal `fullStaticOutput` buffer that NEVER clears (no public API to reset it). When a `key` prop change forces `<Static>` to remount, it treats ALL items as "new" and APPENDS to `fullStaticOutput`, causing duplicate renders. Additionally, Ink renders all `<Static>` content at the TOP of the terminal, regardless of JSX order - so moving content outside `<Static>` puts it below static content. **Solution**: For `/resume` and `/new`, use `useRemount()` to trigger a full Ink app unmount/remount. This is the ONLY way to clear `fullStaticOutput`. See `packages/tui/index.tsx` for the remount loop implementation.
